...
%build
sed -i 's#"1.0.*"#"2.0.*"#' src/HyperFastCgi/Properties/AssemblyInfo.cs
xbuild /tv:2.0 /p:Configuration=Release /target:rebuild /p:SignAssembly=true /p:AssemblyOriginatorKeyFile="../mono.snk"
gacutil -i src/HyperFastCgi/bin/Release/HyperFastCgi.exe -package 2.0 -gacdir .%{_prefix}
sed -i 's#"2.0.*"#"4.0.*"#' src/HyperFastCgi/Properties/AssemblyInfo.cs
...
echo 'exec %{_bindir}/mono $MONO_OPTIONS "%{_prefix}/lib/mono/2.0/HyperFastCgi.exe" "$@"' >> %{buildroot}%{_bindir}/mono-server-hyperfastcgi2
